          ...
          Navigator.of(scaffoldKey.currentContext).pushReplacementNamed('/Pages', arguments: 1);
           ...
           showDialog(
                    ...
                    builder: (ctx) => AlertDialog(
                        content: Text(
S.of(state.context).wrong_email_or_password,
                      ),
                      actions: <Widget>[
                        TextButton(
        ...
        loader.remove();
         showDialog(
                    ...
                    builder: (ctx) => AlertDialog(
                        content: Text(
                      S.of(state.context).this_account_not_exist,
...
S.of(state.context).wrong_email_or_password,
  ...
  showDialog(
                    ...
                    builder: (ctx) => AlertDialog(
                        content: Text(
S.of(state.context).this_email_account_exists,             
                      ),
                      actions: <Widget>[
                        TextButton(
                          child: Text( S.of(state.context).ok,   style: TextStyle(color: Theme.of(state.context).accentColor),),
      ...
      repository.resetPassword(user).then((value) {
        if (value != null && value == true) {
           showDialog(
                    ...
                    builder: (ctx) => AlertDialog(
                        content: Text(
S.of(state.context).your_reset_link_has_been_sent_to_your_email,            
                      ),
                      actions: <Widget>[
                        TextButton(
                          child: Text( S.of(state.context).login,   style: TextStyle(color: Theme.of(state.context).accentColor),),
                          onPressed: () {
   ...
   showDialog(
                    ...
                    builder: (ctx) => AlertDialog(
                        content: Text(
S.of(state.context).error_verify_email_settings,
